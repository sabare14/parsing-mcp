...
def detect_data_row(
    ...
    for row in candidates:
        ...
        features = {
            ...
            "numeric_ratio": row["numeric_ratio"],
            "value_mix": 0.0 if non_empty_count == 0 else clamp(1.0 - row["short_text_ratio"] * row["string_ratio"]),
            "early_after_header": clamp(1.0 - ((distance - 1) / span)),
            "offset_preference": clamp(1.0 - abs(distance - 2.5) / 3.5),
            "unstyled_ratio": 0.0 if non_empty_count == 0 else clamp(1.0 - style_ratio),
            "unique_value_ratio": row["unique_value_ratio"],
